...
function detectRequestingLaterTime(t: string): boolean {
  return /\b(afternoon|later|after lunch|any(thing)? (later|in the afternoon)|pm\b|later in the day|later slot|evening)\b/.test(t) &&
...
function detectIntent(msg: string, session: Session): Intent {
  ...
  return { type, sentiment: scoreSentiment(t), isAffirmative, bookingIntent, flexibleAvailability, availabilityMentioned, timeOfDay, requestingLaterTime };
}
...
function transition(session: Session, intent: Intent): void {
  ...
  const map: Record<State, () => State> = {
    ...
    booking: () => {
      if (hasValidContact) return "conversion";
      return "contact_capture";
    },
    contact_capture: () => {
      if (hasValidContact) return "conversion";
...
function getBusinessDisplay(session: Session): string {
  ...
  if (session.businessType && BUSINESS_DISPLAY_MAP[session.businessType]) return BUSINESS_DISPLAY_MAP[session.businessType];
...
function buildDeterministicReply(session: Session, intent: Intent): string | null {
  const { bookingIntentConfirmed, bookingConfirmed, contactCollected, availabilityMentioned, painPoints } = session;
  ...
  // HANDLE REQUESTING A LATER/DIFFERENT TIME when time already proposed
  if (session.proposedTime && intent.requestingLaterTime && !hasValidContact) {
    const day = availabilityMentioned || (session.proposedTime.split(" ")[0] || "that day").toLowerCase();
  ...
  // HANDLE AFFIRMATIVE AFTER PROPOSED TIME — lock the confirmed time
  if (session.proposedTime && !session.confirmedTime && intent.isAffirmative && !hasValidContact &&
    session.lastAssistantAction === "proposed_time") {
  ...
  // BOOKING COMPLETE: valid contact present → confirm
  if (bookingIntentConfirmed && hasValidContact) {
  ...
  // PARTIAL CONTACT — have some but missing email or phone
  if (bookingIntentConfirmed && (hasPhone || hasName) && !hasEmail) {
    const time = session.confirmedTime || session.proposedTime || "the call";
    const namePart = displayName ? `${displayName}` : null;
    const phonePart = contactCollected.phone;
    const haveParts = [namePart, phonePart].filter(Boolean).join(" and ");
    session.lastAssistantAction = "asked_contact_details";
    return `I've got ${haveParts} for ${time}. What's the best email to attach to the booking?`;
  }

  if (bookingIntentConfirmed && hasEmail && !hasPhone) {
    const time = session.confirmedTime || session.proposedTime || "the call";
    session.lastAssistantAction = "asked_contact_details";
    return `I've got ${contactCollected.email} for ${time}. What's the best phone number to include?`;
  }

  // AVAILABILITY WITH TIME-OF-DAY: propose correct slot
  if (bookingIntentConfirmed && !hasValidContact && availabilityMentioned && !session.proposedTime) {
  ...
  // BOOKING INTENT — no availability yet
  if (bookingIntentConfirmed && !hasValidContact && !availabilityMentioned && !session.proposedTime) {
    session.lastAssistantAction = "asked_time_preference";
...
function buildPrompt(session: Session): string {
  ...
  const knownFacts = [
    session.businessDisplayPhrase ? `- Business: ${businessDisplay}` : (session.businessType ? `- Business type: ${businessDisplay}` : null),
    painPoints.length ? `- Pain points: ${painPoints.join(", ")}` : null,
    availabilityMentioned ? `- Availability: ${availabilityMentioned}` : null,
    session.confirmedTime ? `- Confirmed time: ${session.confirmedTime}` : (session.proposedTime ? `- Proposed time: ${session.proposedTime}` : null),
    contactCollected.name ? `- Name: ${contactCollected.name}` : null,
    contactCollected.email ? `- Email: ${contactCollected.email}` : null,
    contactCollected.phone ? `- Phone: ${contactCollected.phone}` : null,
  ].filter(Boolean).join("\n");
  ...
  const stageInstructions: Record<State, string> = {
    ...
    booking: availabilityMentioned
      ...
      : `They want to book. Ask what day works and get their name, phone, and email in the same message.`,
    contact_capture: (hasPhone || hasEmail)
  ...
  return [
    ...
    `- NEVER restart discovery after booking intent is confirmed.`,
    `- If user said yes to a booking offer, proceed directly to scheduling and contact capture.`,
    `- If user gave time-of-day preference (e.g. "afternoon"), propose a matching time slot (afternoon = 2:00 PM, morning = 10:00 AM).`,
    ...
    `- A valid email must contain @ and a domain like .com. "trustfund.com" alone is NOT a valid email.`,
    `- Max 2–3 sentences. Direct and human.`,
    `- No filler openers: no "Absolutely!", "Great question!", "Of course!", "Certainly!"`,
  ].filter(Boolean).join("\n");
}
...
async function fireWebhook(
  ...
  const payload = {
    ...
    ai_reply: aiReply,
    wants_booking: wantsBooking,
    needs_human: session.bookingConfirmed ? "no" : "yes",
    status,
    scheduled_time: session.confirmedTime || session.proposedTime || "",
    trigger,
  };
